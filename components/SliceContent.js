...
export default function SliceContent({ data }) {
  const slices = data.body.map(function (slice, index) {
    if (slice.slice_type === "rich_text") {
      return (
        <div className="prose prose-lg sm:prose-xl md:prose-2xl lg:prose-3xl mt-12 lg:mt-24 xl:mt-32 container mx-auto max-w-xl md:max-w-2xl lg:max-w-4xl lg:px-16">
          {RichText.render(slice.primary.text, linkResolver)}
        </div>
      );
    } else if (slice.slice_type === "blockquote") {
      return (
        <div className="prose prose-lg sm:prose-xl md:prose-2xl lg:prose-3xl my-12 lg:my-24 container mx-auto max-w-xl md:max-w-2xl lg:max-w-4xl lg:px-16">
